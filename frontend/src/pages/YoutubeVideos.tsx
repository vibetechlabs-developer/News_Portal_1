          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="headline-primary text-foreground">
                {language === 'en' ? 'YouTube Videos' : 'યુટ્યુબ વિડિયો'}
              </h1>
              <p className="text-muted-foreground text-sm">
                {language === 'en' ? 'Watch latest YouTube videos' : 'તાજા યુટ્યુબ વિડિયો જુઓ'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                {language === 'en' ? 'No YouTube videos found.' : 'કોઈ યુટ્યુબ વિડિયો મળ્યા નથી.'}
              </div>
            ) : (
                ...
                {youtubeItems.length > 0 && (
                  <div className="bg-card rounded-xl overflow-hidden border border-border shadow-sm">
                    <div className="relative aspect-video bg-black">
                      ...
                      <button
                        onClick={() => setActiveVideo(youtubeItems[0])}
                        className="absolute inset-0 flex items-center justify-center group"
                      >
                        <div className="w-20 h-20 flex items-center justify-center bg-white/20 backdrop-blur-md rounded-full group-hover:bg-white/30 transition-colors pointer-cursor">
                ...
                {youtubeItems.length > 1 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {youtubeItems.slice(1).map((item) => {
                      ...
                      return (
                        <div key={`${item.source}-${item.id}`} className="relative bg-black rounded-lg overflow-hidden group">
                           <VideoCard
                            thumbnail={
                              thumbnailUrl ||
                              '/logo.png'
                ...
                <div className="bg-card rounded-lg border border-border p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{youtubeItems.length}</span>
                    </div>
                  </div>
                </div>
              )}
